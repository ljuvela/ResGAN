...
def load_data(files_list, data_dir, num_files=30, context_len=64):
...
# function for loading data.nc NetCDF files packaged with Xin's tools
def load_pulselib_data(files_list, data_dir, target_dir,
                       num_files=30, context_len=64):
    ...
    input_data_seqs = np.zeros((len(input_data), context_len, input_dim), dtype=np.float32)
    target_data = np.zeros((len(input_data),), dtype=np.int16)
 
    sample_ind = 0 # running index for sample in full dataset (batch dimension)                  
    start_ind = 0 # running sequence start index in concatenated data matrix 
    for seq_len, seq_tag in zip(seq_lengths, tags):
        # prepend zero context frames to sequence                            
        padded_seq_data = np.vstack((np.zeros((context_len-1, input_dim), dtype=np.float32),
                                              input_data[start_ind:start_ind+seq_len, :]))
        ...
        # read indexes
        idx_file = target_dir + seq_tag + '.idx'
        idxs = np.fromfile(idx_file, dtype=np.int16)
        assert len(idxs) == seq_len
        target_data[start_ind:start_ind+seq_len] = idxs

        start_ind += seq_len

    return target_data, input_data_seqs, tags, seq_lengths



# function for loading data.nc NetCDF files packaged with Xin's tools
def load_test_data(files_list, data_dir, num_files=30, context_len=128):
...
class nc_data_provider:
    ...
    def __init__(self, file_list, data_dir, input_only=False):
        self.i = 0
...
class pulselib_data_provider:

    def __init__(self, file_list, data_dir, target_dir='',
                 input_only=False, context_len=1):
        self.i = 0
        self.file_list = file_list
        self.data_dir = data_dir
    ...
    def next(self):
        if self.i >= len(self.file_list):
            ...
            if self.input_only:
                in_data = load_test_data([self.file_list[self.i]],
                                         self.data_dir,
                ...
                out_data, in_data, _, _ = load_pulselib_data(files_list=[self.file_list[self.i]],
